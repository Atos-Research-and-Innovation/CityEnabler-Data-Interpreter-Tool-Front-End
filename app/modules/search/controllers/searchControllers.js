...
(function() {
    ...
    var searchMainController = function($scope, $location, searchclient, esFactory, API_CONF, Language, PolicyDomain, Individual, Auth, $routeParams, $timeout, $filter) {
        ...
        $scope.sortOptions = [{
            id: 'Relevance',
            name: $filter('translate')('Relevance')
        }, {
            id: 'Title',
            name: $filter('translate')('Title')
        }, {
            id: 'DateDesc',
            name: $filter('translate')('Date Added Desc.')
        }, {
            id: 'DateAsc',
            name: $filter('translate')('Date Added Asc.')
        }];
		...
		*/
        ...
        normalizeAggregationFilter = function() {
        	//console.log("normalizeAggregationFilter")
            var filters = [];
            ...
            angular.forEach(facetsSelected, function (values, facetCategory) {
          	...
          	console.log("add filter by user organisations")
...
//"creator_path": ("0000000" + getUserId()).slice(-7)
        ...
        if (!angular.equals({}, filters)) {
        	...
        	//only do the search if user is lloged
        	if (Auth) {
	          	if (Auth.state) {
        ...
        function getUserId() {
            ...
            return (Auth.state.userPath) ? Auth.state.userPath : -1;
        }
        $scope.ownContentChanged = function ($event) {
            ...
            facetsSelected["userId"] = (checked) ? (getUserId()) : null;
        ...
        $scope.init = function() {
            ...
            userId = ($routeParams.user_id) ? ($routeParams.user_id) : "";
