...
angular.module('pcApp.datasets.controllers.dataset', [
        ...
        'dialogs', '$log', function (dialogs, $log) {
            return {
                ...
                generateTimeSeries: function (resolution, start, end) {
					...
					//to avoid error with hours if start date and end date has different uso
					if (resolution == 'day') {
    ...
    .controller('DatasetEditController', [
        ...
        function ($scope,
            ...
            var getDatasetSuccess = function (dataset) {
				...
				//only if I'm logged, and I'm admin of the organisation that the dataset belongs I can edit. It is in draft and I'm the author, I can edit
       			if ($scope.userState.loggedIn) {
       				//check idf is in draft and I'm the author	
       				if ($scope.userState.isAdmin) {
       					$scope.CanSaveDataset = true;
       				}
       				else if ((dataset.is_draft==true) && (dataset.creator_path==$scope.userState.userPath))
       				{
       					$scope.CanSaveDataset = true;
       				}
       				else {
       					...
       					if (dataset.organization) {
       						...
       						angular.forEach($scope.userState.userData.organizations, function (value, key) {
								...
								if (value.id==dataset.organization) {
						...
						if (Auth.state) {
							if (Auth.state.userData) {
								angular.forEach(Auth.state.userData.organizations, function (valueC, keyC) {
									...
									if (dataset.organization==valueC.id) {
										vTicked = true;
									}
                ...
                var canEdit = Auth.state.isAdmin || $scope.userState.userPath == $scope.dataset.creator_path;
					...
					if ($scope.organizationsList.length > 0) {				
						angular.forEach($scope.organizationsList, function (value, key) {
							...
							if (value.ticked) {
								$scope.dataset.organization = value.maker;
							}
						})
	                }
                
                    if ($scope.dataset.organization=='') {
    ...
    .controller('DatasetStep1Controller', [
        ...
        '$http',
        '$filter',
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, creationService, API_CONF, $http, $filter) {
            $scope.inputTable = creationService.data.inputTable;

            $scope.inputTable.settings.readOnly = false;
            //add context menu by default in step 1
            $scope.inputTable.settings.contextMenu = true;
                ...
                handlers: {
                    ...
                    success: function (file, response) {
                        ...
                        if(response['result'] == 500){
                            ...
                            var label = $filter('translate')('Selected dataset cannot be displayed');
                			var message = $filter('translate')('Please choose another one.');
			...
			$scope.federatedOpenDataImport = {
                isVisible: false,
                toggleVisibility: function () {
                    $scope.federatedOpenDataImport.isVisible = !$scope.federatedOpenDataImport.isVisible;
                    if ($scope.federatedOpenDataImport.isVisible)
                        $scope.dropzone.isCollapsed = true;

                },
                loadData: function (dataset, resource, data) {
                    $scope.federatedOpenDataImport.isVisible = false;
			...
			$scope.unlinksubscription = function() {
					
					
					var label = $filter('translate')('Are you sure?');
                	//var message = $filter('translate')('Do you want to exit without saving this visualization?');
                    var message = 'Do you want to unlink this dataset with the subscription?'             
                    var dlg = dialogs.confirm(label, message);
                    
                    dlg.result.then(function () {
                    	creationService.data.dataset.subscriptionId = '';
                    });
					
					
			}
			
			$scope.importcontextBrockerEnabled = API_CONF.ORION_CONTEXT_BROKER_ENABLE_IMPORT;
			
			$scope.OrionContextBrockerImport= [];
			$scope.OrionContextBrockerImport.isVisible = false;
			$scope.orionContextBrocker = {
                isVisible: false,
                toggleVisibility: function () {
                    $scope.OrionContextBrockerImport.isVisible = !$scope.OrionContextBrockerImport.isVisible;
                    if ($scope.OrionContextBrockerImport.isVisible)
                        $scope.dropzone.isCollapsed = true;
                ...
                loadData: function (dataset, resource, data) {
                    ...
                    creationService.data.dataset.title = creationService.data.dataset.title.replace(/_/g, ' ');
                    
                    
                    creationService.data.dataset.description = (resource.description && resource.description.length > 0) ? resource.description : resource.description;
                    //console.log(data.subscriptionId);
                    creationService.data.dataset.subscriptionId = data.subscriptionId;                    
                    creationService.data.timeResolution = data.frequency;
                    creationService.data.time.start = data.time_start;
                    creationService.data.time.end = data.time_end;
    ...
    .controller('DatasetStep2Controller', [
        ...
        'API_CONF',
        '$filter',
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, creationService, Individual, $http, API_CONF, $filter) {
            ...
            var getSelection = function (startRow, startColumn, endRow, endColumn) {
                ...
                var cntItems = $scope.individualSelection.length;
                angular.forEach(result, function (r) {
                    if ((!_.contains($scope.individualSelection, r)) && r != null && r != '') {
                        searchForIndividuals(r, cntItems);
                        cntItems = cntItems + 1;	
                    }
                });
            };
            ...
            var searchForIndividuals = function(search_term, posItem){
                ...
                success(function (data, status, headers, config) {
                    ...
                    //avoid to add the related items found in the database
                    /*
                    if(data.length > 0) {
                    ...
                    */
                    ...
                    if(suggestions.length > 1){
                        $scope.saveIndividual(search_term, suggestions, "", posItem);
                    }
                    else{
                        $scope.saveIndividual(search_term, suggestions, search_term, posItem);
                    }
            ...
            $scope.saveIndividual = function(key, suggestions, selectedSuggestion, posItem){
                ...
                if(individualIndex >= 0){
                    ...
                    //$scope.individualSelection[posItem] = newIndividual;
                }
            }
            ...
            var init = function () {
                ...
                */
				...
				$scope.datasetData = creationService.data.dataset;
				//console.log($scope.datasetData.subscriptionId)
				if ($scope.datasetData.subscriptionId) {
					$scope.inputTable.settings.readOnly = true;
					$scope.inputTable.settings.afterSelectionEnd = ""
					...
					getSelection(1, 0, 1, 0)
					//searchForIndividuals('Traffic incidents', 0);
            ...
            $scope.nextStep = function () {
                ...
                if ($scope.selection.output.length == 0) {
                	...
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please provide a Data Dimension Type.');
                ...
                if(!allIndividualsChecked()){
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please choose one option for each Dimension.');
                ...
                if (creationService.data.individualSelection.length == 0) {
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please choose at least one Data Dimension.');
    ...
    .controller('DatasetStep3Controller', [
        ...
        'creationService',
        '$filter',
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, creationService, $filter) {
			
			$scope.startDateByResolution = [];
			$scope.endDateByResolution = [];
			$scope.startDateByResolution['day'] = '';
			$scope.startDateByResolution['month'] = '';
			$scope.startDateByResolution['year'] = '';
			$scope.startDateByResolution['quarter'] = '';
			
			$scope.endDateByResolution['day'] = '';
			$scope.endDateByResolution['month'] = '';
			$scope.endDateByResolution['year'] = '';
			$scope.endDateByResolution['quarter'] = '';
			
			$scope.dataset = creationService.data.dataset;
			
			$scope.$watchCollection('timeResolution.output', function (newVal, oldVal) {
				if (oldVal[0] != undefined) {
					
					if (oldVal[0]['id'] != undefined) {
						if (newVal[0]['id']!=oldVal[0]['id']) {
							
							$scope.startDateByResolution[oldVal[0]['id']] = $scope.time.start;
							$scope.time.start = '';
							$scope.time.start = $scope.startDateByResolution[newVal[0]['id']]
							
							$scope.endDateByResolution[oldVal[0]['id']] = $scope.time.end;
							$scope.time.end = '';
							$scope.time.end = $scope.endDateByResolution[newVal[0]['id']]
                    ...
                    input: [
                        {
                            ...
                            regexp: /^(\d{4})(\/|-)([0][1-9]|[1][0-2])\2([0][1-9]|[12][0-9]|3[01])$/
                        }, {
                            name: 'Month',
                            id: 'month',
                            placeholder: '2001-01',
            ...
            $scope.nextStep = function () {
                ...
                if ($scope.timeResolution.output[0] == null) {
                	...
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please provide a Time Resolution.');
                ...
                if ($scope.time.start == null || $scope.time.start == '') {
                	...
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please provide a Start Date.');
                ...
                if ($scope.time.end == null || $scope.time.end == '') {
                	...
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please provide an End Date.');
    ...
    .controller('DatasetStep4Controller', [
        ...
        'Class',
        '$filter',
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, creationService, Class, $filter) {
            ...
            $scope.autofillTable = function(){
                ...
                if(!foundMatchingItems){
                	...
                	var message = $filter('translate')('Could not fill table automatically');
                    
                    message = message +"<br>"+ $filter('translate')('helping_message_step_4');        
                    dialogs.notify(label, message);
            ...
            $scope.nextStep = function () {
				...
				*/
                if ($scope.unit.output.length == 0) {
                	...
                	var label = $filter('translate')('Validation Error');
                	var message = $filter('translate')('Please provide an Unit.');
                ...
                creationService.data.indicator = '';
    ...
    .controller('DatasetStep7Controller', [
        ...
        'ExternalResource',
        'Auth',
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, creationService, $filter, Dataset, $location, Individual, ExternalResource, Auth) {
            ...
            var preSave = function () {
                ...
                payload.subscription_id = $scope.dataset.subscriptionId;
            ...
            var saveErrorCallback = function (err) {
                ...
                if (headers['content-type'] == 'text/html') {
                    //dialogs.error("Internal Server Error", "Please contact the Administrators.");
            ...
            var saveFinish = function () {
				...
				if ($scope.organizationsList.length > 0) {				
					angular.forEach($scope.organizationsList, function (value, key) {													
							if (value.ticked) {
								payload.organization = value.maker;
							}
					})
				}
    ...
    .controller('DatasetDetailController', [
        ...
        function ($scope, DatasetsControllerHelper, $log, dialogs, ngProgress, $routeParams, $filter, Dataset, Individual, $q, Indicator, $location, Auth, VisualizationByDataset) {
            ...
            var getDatasetSuccess = function (dataset) {
				...
				//only if I'm logged, and I'm admin of the organisation that the visualization belongs I can edit. It is in draft and I'm the author, I can edit
       			$scope.DataLoaded = true;
       			if ($scope.userState.loggedIn) {
       				//check idf is in draft and I'm the author	
       				if ($scope.userState.isAdmin) {
       					$scope.CanViewItem = true;
       					$scope.CanDeleteDataset = true;
       				}
       				else if ((dataset.is_draft==true) && (dataset.creator_path==$scope.userState.userPath))
       				{
       					$scope.CanViewItem = true;
            ...
            $scope.deleteDataset = function (dataset) {
                if($scope.relatedVisualizationsString.length == 0){
                    ...
                    var label = $filter('translate')('Are you sure?');
                	var message = $filter('translate')('Do you really want to clear the Dataset permanently?');
                	...
                	var label = $filter('translate')('Cannot delete this dataset');
                	var message = $filter('translate')('This dataset is being used by one or more visualizations. Please delete these visualizations first');
                	/*
